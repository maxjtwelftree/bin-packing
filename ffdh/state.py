...
penalty_factor = 1000  # Adjust this value as needed
...
# Clone the state (deepcopy)
    def clone(self):
        return State(
            ...
            {layer: intervals.copy() for layer, intervals in self.available_spaces.items()}
        )
    ...
    def evaluation(self):
        total_area = sum((action[0].width * action[0].height for action in self.action_history))
        num_boxes_placed = len(self.action_history)
        # Subtract the number of boxes placed to prioritize fewer boxes
        return total_area - (num_boxes_placed * penalty_factor)
